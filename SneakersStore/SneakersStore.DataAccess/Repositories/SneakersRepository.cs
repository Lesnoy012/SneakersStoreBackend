...
namespace SneakersStore.DataAccess.Repositories
{
    public class SneakersRepository : ISneakersRepository
    {
        ...
        public async Task<List<Sneakers>> Get(SneakersFilter filter, SneakersSort sort)
        {
            ...
            var direction = sort.SortDirection;

            var sortedQuery = (sortBy, direction) switch
            {
                ("title", null) => query.OrderBy(s => s.Title),
                ("price", SortDirection.Ascending) => query.OrderBy(s => s.Price),
                ("price", SortDirection.Descending) => query.OrderByDescending(s => s.Price),
                _ => query.OrderBy(s => s.Title)
            };

            var sneakersEntities = await sortedQuery.ThenBy(s => s.Id).ToListAsync();
